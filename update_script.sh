...
# Retry certbot if it fails
RETRIES=3
for (( i=1; i<=$RETRIES; i++)); do
    certbot certonly --standalone --non-interactive --agree-tos --email $EMAIL -d $DOMAIN && break
    if [ $i -lt $RETRIES ]; then
        echo "Certbot failed. Retrying in 30 seconds..."
        sleep 30
    else
        echo "Certbot failed after $RETRIES attempts. Exiting."
        exit 1
    fi
done
...
# Reminder to check logs via Apache
echo "To access the logs via a web browser, navigate to http://$DOMAIN/logs/ocserv.log"
echo "Use the username 'admin' and the password you set when prompted."
