...
# Update the system and install necessary packages in the background
{
    apt update
    apt upgrade -y
...
# Request a Let's Encrypt certificate in the background
{
    certbot certonly --standalone --non-interactive --agree-tos --email $EMAIL -d $DOMAIN
} &
CERTBOT_TASK_PID=$!

# Wait for the system update and installation to finish
wait $BG_TASK_PID
...
# Wait for the certbot to finish
wait $CERTBOT_TASK_PID
