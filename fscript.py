...
apt install ocserv certbot gnutls-bin netfilter-persistent iptables-persistent -y
...
# Check for certificate generation success
if [ $? -ne 0 ]; then
    echo "Error obtaining certificate. Exiting."
    exit 1
fi
...
# Create or update ocserv user
echo "$OCPASS" | ocpasswd -c /etc/ocserv/ocpasswd "$OCUSER"

# Ensure /run directory exists
[ -d "/run" ] || mkdir /run

# Set permissions for the ocserv socket
chown -R nobody:nogroup /run/ocserv.socket

# Stop ocserv service and remove any old socket files
systemctl stop ocserv
rm -f /run/ocserv.socket*
